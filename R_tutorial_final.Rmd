...
#### First download the data from the github repository at [link](data/03_neurites_synthetic.csv)
...
# Hint: if you can't locate the file use function getwd() to figure out where is your local working directory
```
...
# This returns a vector of values from condition_1 column
# vector is a basic data structure often used in R
```
...
If we merge several normal distributions we typically don't get normally distributed data. We can look at such example by mixing the data from our 3 conditions together

```{r}
# Merge the data from all columns into single vector
...
In summary our dataset contains samples with: 
