...
Download the file "GSE192855_RAW.tar" and place it into the folder "data"

![](images/GEO_screenshot.jpg)


------------- 
Extract the archive, you should see 19 files:

![](images/GEO_unpacked.jpg)

### Question 1: How many timepoints and how many replicates per each timepoint are there in this dataset ? 

Hint: Use the information provided in the [GEO page](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE192855) 

Can you tell from filenames which replicate and timepoint it corresponds to ? 
...
# Here you will need to change the correct name and exact path of your file (path can be absolute or relative to the path of the notebook)
...
```{r}
# Now we only extract the counts and gene ids and create a vector
# Column 1 contains Gene IDs, and Column 7 contains raw counts.

D0_rep1_counts        <- D0_rep1_table[,7] # This extracts vector of counts
names(D0_rep1_counts) <- D0_rep1_table[,1] # This extracts gene ids
...
D0_rep2_counts        <- D0_rep2_table[,7] # This extracts vector of counts
names(D0_rep2_counts) <- D0_rep2_table[,1] # This extracts gene ids
...
```{r}
# Advanced and optional: Alternatively one can use a for loop to load all the files into a list of vectors. 
# Note: There is some deliberate typos in this part, try to catch them and fix
...
#   counts_list[[file]]        <- counts_tble[,7] # This extracts vector of counts
...
Count matrix is the essential input into DESeq2 analysis. Count matrix is composed of genes (rows) and samples (columns). Count matrix can be generated in different ways, this is just one example how to get there from this particular structure of data deposited in GEO.  
...
Note: It is critical that the column names of the count matrix and row names of metadata are in the same order. E.g. column 1 of count matrix corresponds to row 1 of metadata etc. 
...
coldata <- data.frame(
            name = colnames(combined_counts),
            timepoint = as.factor(c('D0','D0','D0','D0','D0','D0',
                                    'D7','D7','D7','D7','D7',
                                    'D13','D13','D13',
                                    'D20','D20','D20','D20','D20')),
            replicate = c(1,2,3,4,5,6,
                          1,2,3,4,5,
                          1,2,3,
                          1,2,3,4,5))
...
### Question 3: Does the metadata we created manually - timepoint and replicate number correspond to the actual samples ? 

 - If it does not fix the metadata 
...
#### Visualize the library sizes for each replicate
...
```
# Converting ENSG codes to gene names
...
ggplot(data = res, aes(x = log2FoldChange, y = -log10(padj), color = sig)) + 
  geom_point(size = 0.02, alpha = 0.7) +              # Use appropriate point size and transparency
  scale_color_manual(values = c("gray", "darkred")) +  # Assign colors (e.g., gray for non-sig, red for sig)
  labs(
    title = "Volcano Plot",
    x = "Log2 Fold Change",
    y = "-Log10 Adjusted P-value",
  ...
  theme(
    legend.position = "top",                       # Place legend at the top
    plot.title = element_text(hjust = 0.5)         # Center the plot title
  )  + geom_text_repel(data=res[which(res$padj < 1e-50 & abs(res$log2FoldChange) > 5),],aes(x=log2FoldChange,y=-log10(padj),label=gene_name))
```
