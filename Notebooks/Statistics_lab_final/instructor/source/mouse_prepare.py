...
if cfg['variant'] == 'mouse':
    ...
    measurement_rows = [dict(sample_id=r['MouseID'].rsplit('_',1)[0],
        ...
        NR1=r['NR1_N'] if r['NR1_N'] != '' else 'NA') for r in records]
    ...
    core = [dict(sample_id=r['sample_id'], group=r['treatment'], BDNF=r['BDNF'],
                 pCREB=r['pCREB'], NR1=r['NR1'], genotype=r['genotype'],
                 learning=r['learning'], source_record=r['source_record']) for r in rows]
    assert len(core) == 72 and len({r['sample_id'] for r in core}) == 72
...
measurements = ('BDNF','pCREB','NR1') if cfg['variant']=='mouse' else ('reversal_mV','resting_mV')
