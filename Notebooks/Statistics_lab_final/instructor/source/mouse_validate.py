...
assert list(core[0]) == ['sample_id','group','BDNF','pCREB','NR1','genotype','learning','source_record']
...
if cfg['variant']=='mouse':
 ...
 for r in core:
  ...
  assert len(core)==72 and r['group']==v['treatment']
  assert all(r[k]==v[k] for k in ['genotype','learning','source_record'])
  eq(r['BDNF'],v['BDNF']); eq(r['pCREB'],v['pCREB']); eq(r['NR1'],v['NR1'])
else:
 import openpyxl
 w=openpyxl.load_workbook(source,read_only=True,data_only=True); s=w['Figure 1G+1H']; cells=rows('cells.csv')
 assert len(cells)==47 and len(full)==26
 for r in cells:
