...
## In this notebook you will:
 ...
 - Learn simple data vizualisation in base R
...
1. Increase length values of *lengths_um* by 10% (e.g., expected growth).

2. Apply a microscope calibration: multiply each value of *lengths_um* by a scale factor 0.98 and subtract a blank offset of 0.15 µm.

3. Return the indices (and values) where the calibrated lengths are > 15 µm.

4. Calculate z-score of lengths_um. 

Note: Z-score is a measure of how many standard deviations a value is from the mean of its distribution.
...
z = (x - μ) / σ
 ...
 - z: is the z-score
 - x: is the raw data point or individual score
...
1. Create an empty folder on your desktop (or any preferred location) and name it `statistical_analysis_R`.
...
3. Create a new R notebook and save it into the notebooks folder
...
#### Exercise: 
 ...
 - Compute the same statistics (mean, sd, quantiles and median) for the other 2 columns condition_1 and condition_2 
...
#### Comparing data in columns: Student's t test of condition 1 agains condition 2 (column condition_1 vs column control)
...
#### Exercise: 
 - Print neurite data frame rows where control is greater than 0.2 and condition_1 is greater than 1
...
#### Exercise: 
 ...
 - plot histograms of condition_1 and condition_2 columns of neurite data frame
...
#### Exercise: 
 - plot only 3 columns from neurites data.frame - control, condition_1 and condition_2

------

## Homework - independent exercises
...
2. Compute the mean of vector c(1.15, 1.17, 0.83, NA, 1.43, 0.58) with and without na.rm=TRUE parameter. Explain the difference.
3. Rename columns of data frame neurites condition_1 → cond1, condition_2 → cond2 using names().
...
9. Plot histogram of column delta_1 of df neurites
10. Plot histogram of log normalized values of condition_1
